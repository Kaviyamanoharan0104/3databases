...
# # 🔹 Function to Load and Decrypt JSON
# def load_and_decrypt_json(encrypted_json_path, key_path):
#     # Load encryption key
...
#     encrypted_data["private_key_id"] = cipher_suite.decrypt(encrypted_data["private_key_id"].encode()).decode()
#     encrypted_data["private_key"] = cipher_suite.decrypt(encrypted_data["private_key"].encode()).decode()

#     # Save decrypted JSON temporarily
#     decrypted_json_path = "decrypted_json.json"
#     with open(decrypted_json_path, "w") as file:
#         json.dump(encrypted_data, file, indent=4)

#     return decrypted_json_path
...
#              # 🔹 Load and Decrypt Service Account JSON
#              decrypted_json_path = load_and_decrypt_json("encrypted_json.json", "encryption_key.key")

#              credentials = service_account.Credentials.from_service_account_file(decrypted_json_path)

#              # Create the SQLAlchemy engine URL with the service account credentials
#              engine_url = f"bigquery://{project_id}/{dataset_name}?credentials_path={decrypted_json_path}"
